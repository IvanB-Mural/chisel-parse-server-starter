...
const getMyTalks = async (participant, siteId) => {
  try {
    ...
    if (siteNameId === null)
      return { status: "error", message: "Invalid siteId" };
...
Parse.Cloud.define("joinTalk", async (request) => {
  ...
  try {
    ...
    if (!talkWrapParseObject) {
      ...
      const talkWrapName = `${title}-${new Date(
        start
      ...
      const alreadyBooked = participantIds.reduce(
        (acc, cur) => acc || cur.id === participantId,
        false
      );
      if (alreadyBooked) {
      ...
      isJoinable =
        isNaN(max_capacity) ||
        max_capacity === 0 ||
        max_capacity > participantIds.length;
      if (!isJoinable) {
  ...
  for (let model of models) {
    ...
    getTableData(tableName).then((response) => {
      ...
      if (!deleting) {
        ...
        if (CLP["get"].hasOwnProperty(user.id)) delete CLP["get"][user.id];
        if (CLP["find"].hasOwnProperty(user.id)) delete CLP["find"][user.id];
      }
...
Parse.Cloud.beforeSave(`Model`, async (request) => {
  ...
  for (let collab of collabs) {
    ...
    if (role == ROLE_ADMIN) admins.push(user.id);
